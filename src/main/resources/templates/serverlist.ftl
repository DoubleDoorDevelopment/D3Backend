...
<table class="table table-hover">
    ...
    <#list Settings.servers as server>
        ...
        <tr class="<#if server.online>success<#else>danger</#if>" style="cursor:pointer;">
            <td onclick="window.document.location='/servers/${server.ID}'">${server.ID}</td>
            <td onclick="window.document.location='/servers/${server.ID}'">${server.displayAddress}</td>
            <td onclick="window.document.location='/servers/${server.ID}'">${server.onlinePlayers}/${server.slots}</td>
            <td onclick="window.document.location='/servers/${server.ID}'">${server.diskspaceUse[2]} MB</td>
            <td>
                <div class="btn-group">
                    <button type="button" <#if !server.online>onclick="call('server', '${server.ID}', 'startServer')" <#else>disabled</#if> class="btn btn-success btn-xs">Start</button>
                    <button type="button" class="btn btn-info btn-xs" onclick="openPopup('/console/${server.ID}')">Console</button>
            ...
            <td onclick="window.document.location='/servers/${server.ID}'">${server.motd}</td>
        </tr>
        </#if>
    </#list>
    </tbody>
