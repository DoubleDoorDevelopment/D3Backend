...
<div class="panel panel-info">
    ...
    <div class="panel-body">
        <div class="btn-group">
            <button type="button" onclick="{var n = prompt('New file name?', ''); if (n != null) call('filemanager/${fm.server.ID}/${fm.stripServer(fm.file)}', 'newFile', [n], reload);}" class="btn btn-default btn-xs">New file</button>
            <button type="button" onclick="{var n = prompt('New folder name?', ''); if (n != null) call('filemanager/${fm.server.ID}/${fm.stripServer(fm.file)}', 'newFolder', [n], reload);}" class="btn btn-default btn-xs">New folder</button>
    ...
    <table class="table table-hover table-condensed tablesorter" id="servers">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <div class="btn-group">
                        <button type="button" onclick="{var n = prompt('New file name?', '${file.getName()}'); if (n != null) call('filemanager/${fm.server.ID}/${fm.stripServer(file)}', 'rename', [n], reload);}" class="btn btn-default btn-xs">
                            Rename
                        </button>
                        <button type="button" onclick="if (confirm('Are you sure?\nIt will be gone FOREVER!')) call('filemanager/${fm.server.ID}/${fm.stripServer(file)}', 'delete', [], reload);" class="btn btn-danger btn-xs">Delete</button>
                        <#if !file.canWrite()>
                        ...
                        <#if fm.getExtension(file) == "jar" || fm.getExtension(file) == "zip">
                            <button type="button" onclick="call('filemanager/${fm.server.ID}/${fm.stripServer(file)}', 'rename', ['${file.getName()}.disabled'], reload);" class="btn btn-default btn-xs">Disable</button>
                        <#elseif fm.getExtension(file) == "disabled">
                            <button type="button" onclick="call('filemanager/${fm.server.ID}/${fm.stripServer(file)}', 'rename', ['${file.getName()?replace(".disabled", "")}'], reload);" class="btn btn-default btn-xs">Enable
                            </button></#if>
                    </div>
                </td>
            </tr>
...
<script>
    ...
    //fileForm.setAttribute("action", "filemanager/${fm.server.ID}/${fm.stripServer(fm.file)}");
